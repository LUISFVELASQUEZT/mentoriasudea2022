...
def ejecutar_consulta_un_registro (conexion, comando):
    cursor = conexion.cursor()
    resultado = None
    try:
        cursor.execute(comando)
...
def show_results(conexion, comando,msg):
    ...
    print(f'{"F" * len(msg)} Inicio de {msg} {"F" * len(msg)}')
...
comando="SELECT * from padres where cedula='Z99999'"
respuesta=ejecutar_consulta_un_registro (conexion, comando)        
if respuesta == None:
    print("not found")
else:
    print(respuesta)      
